	...
	<div class="intellectual-dashboard">
		...
		<el-row :gutter="10">
			<el-col :lg="8" :md="12" :xs="24">
				<count-card :title="t('已获得专利数量')" icon="document" :value="patentGranted.value"
					:trend-text="`${patentGranted.growthRate >= 0 ? '+' : ''}${patentGranted.growthRate}%`"
					:footer-label="t('年度新增')" :footer-value="patentGranted.yearlyIncrease" />
			</el-col>
			<el-col :lg="8" :md="12" :xs="24">
				<count-card :title="t('申请中的专利数量')" icon="clock" :value="patentApplying.value"
					:trend-text="`${patentApplying.growthRate >= 0 ? '+' : ''}${patentApplying.growthRate}%`"
					:footer-label="t('年度新增')" :footer-value="patentApplying.yearlyIncrease" />
			</el-col>
			<el-col :lg="8" :md="12" :xs="24">
				<count-card :title="t('今年授权的专利数量')" icon="trophy" :value="patentYearly.value"
					:trend-text="`${patentYearly.growthRate >= 0 ? '+' : ''}${patentYearly.growthRate}%`"
					:footer-label="t('较去年同期')" :footer-value="`+${patentYearly.yearlyProgress}`" />
			</el-col>
		</el-row>
		...
		<el-row :gutter="10">
			<el-col :lg="8" :md="12" :xs="24">
				<count-card :title="t('已获得软著数量')" icon="code" :value="softGranted.value"
					:trend-text="`${softGranted.growthRate >= 0 ? '+' : ''}${softGranted.growthRate}%`"
					:footer-label="t('年度新增')" :footer-value="softGranted.yearlyIncrease" />
			</el-col>
			<el-col :lg="8" :md="12" :xs="24">
				<count-card :title="t('年度计划申请软著数量')" icon="calendar" :value="softPlanned.value"
					:trend-text="`${softPlanned.growthRate >= 0 ? '+' : ''}${softPlanned.growthRate}%`"
					:footer-label="t('年度新增')" :footer-value="softPlanned.yearlyIncrease" />
			</el-col>
			<el-col :lg="8" :md="12" :xs="24">
				<count-card :title="t('年度授权软著数量')" icon="medal" :value="softYearly.value"
					:trend-text="`${softYearly.growthRate >= 0 ? '+' : ''}${softYearly.growthRate}%`"
					:footer-label="t('较去年同期')" :footer-value="`+${softYearly.yearlyGrowth}`" />
			</el-col>
		</el-row>
		...
		<el-row :gutter="10">
			<el-col :lg="12" :xs="24">
				<common-type-chart :title="t('专利类型分布')" :data="patentTypeData" :legend-data="[t('已授权'), t('申请中')]"
					:colors="['#67C23A', '#409EFF']" />
			</el-col>
			<el-col :lg="12" :xs="24">
				<common-trend-chart :title="t('专利年度趋势')" :data="patentTrendData"
		...
		<el-row :gutter="10">
			...
			<el-col :lg="12" :xs="24">
				<common-trend-chart :title="t('软著年度趋势')" :data="softTrendData" :series-config="softTrendSeriesConfig" />
			</el-col>
		</el-row>
	</div>
	<!-- </el-scrollbar> -->
...
<script lang="ts" setup>
	...
	const getPatentStatistics = async () => {
		try {
			...
			const applyingPatents = patents.filter(
				patent => patent.legalStatus && patent.legalStatus >= 10 && patent.legalStatus < 100
			);
			...
			patentYearly.yearlyProgress = Math.min(
				Math.round((patentYearly.value / yearlyTarget) * 100),
				100
			);
		} catch (error) {
	...
	const initDictData = async () => {
		const dictTypes = ['intellectual_patent_type', 'intellectual_software_category'];
